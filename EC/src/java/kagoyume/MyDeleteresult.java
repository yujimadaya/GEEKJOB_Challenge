...
public class MyDeleteresult extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{            
            ...
            UserDataDTO loginDTO = (UserDataDTO)hs.getAttribute("loginDTO");    //ログイン時のデータをUserDataDTO(loginDTO)に格納             
            
            UserDataDAO.getInstance().delete(loginDTO);                         //削除処理（deleteFlgに1を格納)
                                    
            k_helper.DeleteSession(hs);                                               //login情報の削除
            //mydeleteresult.jspへ移動
